  ...
  <div id="buff">
    <el-form ref="form" class="el-col-lg-12 el-col-lg-offset-6 el-col-sm-24">
      <el-form-item label="直接输入Buff代码解析">
        ...
        <el-button @click="importBuff" type="success" round v-loading.fullscreen.lock="fullscreenLoading">导入</el-button>
      </el-form-item>
      ...
      <el-card header="Buff属性">
        ...
        <el-form-item label="效果持续的回合数，0为永远存在">
          <el-input-number v-model.number="buff.minTurn" size="small"></el-input-number>
          <span>回合~</span>
          <el-input-number v-model.number="buff.maxTurn" size="small"></el-input-number>
          <span>回合</span>
        </el-form-item>
      ...
      <template v-for="(effect,index) in buff.effects">
        <el-card :key="'effectIndex'+index">
          <div slot="header">
            ...
            <el-button type="danger" icon="el-icon-delete" circle style="float: right"
          ...
          <el-form-item label="Buff效果类型">
            <el-select v-model="buff.effects[index].accumulate" @change="clearValue(index)">
              <el-option
          ...
          <template v-if="effect.accumulate!=7&&effect.accumulate!=10">
            <el-form-item label="每回合造成的伤害或者回复的数值">
              <el-input-number v-model.number="buff.effects[index].value1" size="small"></el-input-number>
              <span v-if="effect.percent==1">%
              </span>
              <span>到</span>
              <el-input-number v-model.number="buff.effects[index].value2" size="small"></el-input-number>
              <span v-if="effect.percent==1">%
              </span>
            </el-form-item>
            <el-form-item label="效果累加极限值" v-if="['0','1','4'].indexOf(effect.accumulate)>-1">
              <el-input-number v-model.number="buff.effects[index].valueLimit"></el-input-number>
              <span v-if="effect.percent==1">%
              </span>
            </el-form-item>
          ...
          <template v-else>
            <el-form-item :label="effect.accumulate==7?'成功后触发':'限制数值触发'">

              <el-select v-if="buff.effects[index].exchangeValue"
                         popper-class="el-col-13"
                         placeholder="输入关键词搜索,空格显示全部"
                         class="el-col-12"
                         filterable :filter-method="querySkillCondition"
                         v-model="buff.effects[index].value1">
                <el-option v-for="condition in querySkillConditions"
                           :label="condition.name"
                           :key="condition.id"
                           :value="condition.id">
                  <span style="float: left;">{{condition.name}}</span>
                  <span style="float: right;font-size: 0.7rem;text-overflow:ellipsis">{{condition.description?condition.description.slice(-70+condition.name.length):''}}</span>
                </el-option>
              </el-select>
              <el-select v-else
                         popper-class="el-col-13"
            ...
            <el-form-item label="极限数值" v-if="buff.effects[index].accumulate==10">
              <el-input-number v-model.number="buff.effects[index].valueLimit"></el-input-number>
              <span v-if="effect.percent==1">%
              </span>
            </el-form-item>
            <el-form-item>
              ...
              :active-text="effect.accumulate==7?'给自己':'数值以下触发'"
              :inactive-text="effect.accumulate==7?'给敌人':'数值以上触发'"
      ...
      <el-form-item>
        <el-button
          type="primary"
          size="small"
          @click="buff.effects.push({type: '',accumulate: '',percent: 1,value1: '',value2: '',valueLimit: 0,exchangeValue:true})">
          增加Buff效果
        </el-button>
      </el-form-item>
      <el-form-item>
        <el-button type="success" icon="el-icon-circle-check" @click="generateBuff"
                   v-loading.fullscreen.lock="fullscreenLoading">生成Buff
        </el-button>
      </el-form-item>
    ...
    export default {
      ...
      data() {
        return {
          ...
          buffResult: '',
          buffTypeInfo: {
            0: "正面技能", 1: "负面技能", 2: "具有正面效果的攻击技能", 3: "具有负面效果的攻击技能", 4: "连击技能",
          },
          buffTargetInfo:{
            0:'自己',1:'敌人',2:'友军'
          },
          buffEffectTypeList:{},
          buffAccumulate:{
            0:"留空",1:"叠加类技能比如四御",4:"自身固定buff",5:"周围友军",6:"敌人靠近时触发",7:"成功时触发另一个技能",9:"周围敌人",10:"限制数值时触发另一个技能",
          },
          conditions:[],
      ...
      methods:{
          importBuff(){
            ...
            if(result.length>15){
              ...
              [this.buff.id,this.buff.name,this.buff.desc,this.buff.sIcon,this.buff.type,this.buff.target,this.buff.minTurn,this.buff.maxTurn,this.buff.removeByTurn,this.buff.removeOnAttack]=result.slice(0,10);
              this.buff.effects=[];
          ...
          generateBuff(){
            this.fullscreenLoading=true;
            //生成Buff代码
            let result = [this.buff.id,this.buff.name,this.buff.desc,this.buff.sIcon,this.buff.type,this.buff.target,this.buff.minTurn,this.buff.maxTurn,this.buff.removeByTurn,this.buff.removeOnAttack];
          ...
          if (query == "")
            ...
            this.querySkillConditions = this.conditions.filter(
              item => {
      ...
      mounted(){
        const buff = this;
        this.$http.get('static/data/BuffEffectTypeInfo.txt').then(res => {
          buff.buffEffectTypeList=res.body
        });
        this.$http.get('static/data/BattleConditionSE.txt').then(res => {
          buff.conditions = dataProcess.text2Dict(res.body);
        })
      }
    }
</script>
